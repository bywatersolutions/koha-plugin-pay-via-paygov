...
| [`install()`](https://github.com/bywatersolutions/koha-plugin-pay-via-paygov/blob/f70ed61f6f6188cd02ba7fc5192b6e77d2358db5/Koha/Plugin/Com/ByWaterSolutions/PayViaPayGov.pm#L405) builds the `CREATE TABLE` for `paygov_plugin_tokens` and **never executes it** — the token INSERT dies unless the table was created by hand. | Whether payments work at all on a fresh install; no card data involved. | Open |
